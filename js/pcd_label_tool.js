...
function addBoundingBoxGui(bbox, bboxEndParams) {
    ...
    let cubeWidth = folderSize.add(bbox, 'width').name("width").min(0.3).max(100).step(0.01).listen();
    let cubeLength = folderSize.add(bbox, 'length').name("length").min(0.3).max(100).step(0.01).listen();
    let cubeHeight = folderSize.add(bbox, 'height').name("height").min(0.3).max(50).step(0.01).listen();
    folderSize.close();
